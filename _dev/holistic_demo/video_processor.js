...
class VideoProcessor
{
  ...
  setupCanvases(width, height)
  {
    ...
    // canvas over to it.
    if (this.useGlCanvas_ && this.processor_ && this.processor_.setGlCanvas && !this.remoteCanvas_)
    {
  ...
  if (!canvas)
  {
    console.error('Canvas is null or undefined in initialize method');
    throw new Error('Invalid canvas parameter');
  }

  // Process video directly on passed-in canvas, for normal demos.
  if (this.config_.displayIsRemote)
  {
    // Process video on separate canvas on main thread.
    this.canvas_ = new OffscreenCanvas(canvas.width ?? 0, canvas.height ?? 0);
    // Keep a reference to the display canvas, for passing over to worker later.
    this.remoteCanvas_ = canvas;
  } else
  {
    this.canvas_ = canvas;
  }

  console.log('Canvas dimensions:', this.canvas_.width, this.canvas_.height);

  const kWidth = this.canvas_?.width ?? DEFAULT_WIDTH;
  const kHeight = this.canvas_?.height ?? DEFAULT_HEIGHT;

  this.video_ = document.querySelector('.video');
  if (!this.video_)
  {
...
setFrameProcessor(frameProcessor) {
  ...
  if (this.useGlCanvas_ && this.processor_.setGlCanvas && this.canvas_ && !this.remoteCanvas_)
  {
    this.processor_.setGlCanvas(this.canvas_);
  }
}
...
getVideoSize(event) {
  ...
  // Can only set once and can't resize, so we do it here.
  if (this.useGlCanvas_ && this.processor_ && this.processor_.setGlCanvas && this.remoteCanvas_)
  {
    // Set remoteCanvas size to be whatever our canvas is right now.
    this.remoteCanvas_.width = this.canvas_.width;
    this.remoteCanvas_.height = this.canvas_.height;
    this.processor_.setGlCanvas(this.remoteCanvas_);
  }
}
...
tick(timestamp) {
    ...
    !this.video_.paused && !this.video_.ended)
  {
    ...
    if (processFrame)
    {
      if (this.onVideoFrameListener_)
      {
        this.onVideoFrameListener_(this.video_);
      }
...
/**
 * Any additional processing we want to be handled in our main program during
 * each video frame can be done so by setting a listener here.
 * @param onVideoFrameListener Function which takes an HTMLVideoElement and
 * a number (video, timestamp), and will be called for every new video frame.
 */
setOnVideoFrame(onVideoFrameListener) {
  this.onVideoFrameListener_ = onVideoFrameListener;
}
