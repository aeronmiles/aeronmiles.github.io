...
float noise(vec3 p, float iterations) {
    float n = 0.;
    float amp = 1.;
    vec4 p4 = vec4(p, 11.);
    p4.xy *= rot(1.4);
    for(float i = 0.; i < iterations; i++) {
        ...
        p4.xz *= rot(2.5 + i);
        p4.wy *= rot(1.5 - i);
        p4 += cos(p4 + vec4(3, 2, 1, 1.)) * amp * .5;
        n += dot(sin(p4), cos(p4)) * amp;
        amp *= 0.7;
        p4 *= 1.5;
    }
    return sin(n * 2.);
...
vec3 hsv2rgbSmooth(vec3 hsv) {
    ...
    return hsv.z * mix(vec3(1.0), rgb * rgb * (3.0 - 2.0 * rgb), hsv.y);
}

void main() {
    vec2 uv = (U - 0.5 * R) / max(R.x, R.y);
    vec3 col = vec3(0);
    ...
    // Clouds
    {
        for(float i = 0.; i < 56.; i++) {
            vec2 cpos = vec2(-mod(i * 12.6 + .05 * iTime * max(abs(sin(i + sin(i * 20.) * 4. + 200.)), 0.8) * 1.05 + scroll * 0., 2.) * 2. + 2. + scroll, cos(i * 20.) * 0.1 + 0.07);
            vec2 lcpos = cpos - p;
...

    {
        for(float i = 0.; i < 50.; i++) {
            ...
            if(sdplank < 0.004) {
                rocks = mix(vec3(0.34, 0.35, 0.4), vec3(0.1) * 2., 1. - step(clamp(dot(lp, normalize(vec2(1, -1))) * 44. + 0.4, 0., 1.), dith));
            }
        }
...

    {
        ...
        for(float i = 0.; i <= griters; i++) {
            ...
            if(lsd < 0. + grass) {
                ...
                grcol *= 1. - vec3(0.6, 0.8, 0.6) * 0.2 * step(dn, 0.5) - vec3(0.6, 0.4, 0.5) * 0.3 * step(dn + 0.9 - grass * 0.7 * (1. - iterscale) + n * 20., 0.1) * (sc * 0.6 + 0.4) - smoothstep(0.02, 0., minDrocks) * 0.4
