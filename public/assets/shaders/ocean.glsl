...
#define WATER_DEPTH .50 // how deep is the water
#define CAMERA_HEIGHT 2.5 // how high the camera should be
#define ITERATIONS_RAYMARCH 12.0 // waves iterations of raymarching
#define ITERATIONS_NORMAL 12.0 // waves iterations when calculating normals
#define TIME_SAMPLES 1.0
#define SHUTTER (1.0 / 24.0)
...
vec3 normal(vec2 pos, float e, float depth, float iterNorm) {
  ...
  return normalize(
    cross(
      a - vec3(pos.x - e, getwaves(pos.xy - ex.xy, iterNorm) * depth, pos.y), 
      a - vec3(pos.x, getwaves(pos.xy + ex.yx, iterNorm) * depth, pos.y + e)
    )
  );
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  for (float s = 0.0; s < TIME_SAMPLES; s++)
  {
      ...
      float d = sqrt(distance(origin, highHitPos)) * 0.15;
      ...
      //float crest = pow(1.0 + waterHitPos.y, 12.0);
      //C = vec3( (ITERATIONS_RAYMARCH / d) / ITERATIONS_RAYMARCH,  (ITERATIONS_NORMAL / d)/ ITERATIONS_NORMAL, 0.0);
      //C = mix(C + crest * 24.0, C, min(0.9, sqrt(dist) * 0.1));
      //d = min(1.0, dist * 0.21);
      //C += mix(crest * 4., 0.0, d);
  }
  
  fragColor = vec4(aces_tonemap(C * (2.0 / TIME_SAMPLES)), 1.0);
  //float w = getwaves(waterHitPos.xz, ITERATIONS_NORMAL) * WATER_DEPTH;
  //fragColor = vec4(w,w,w,1);
  //fragColor = vec4(N.z,0,0, 1);
}